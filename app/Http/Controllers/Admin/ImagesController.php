...
class ImagesController extends Controller
{
    ...
    public function addImage(Request $request, Page $pageModel, Category $categoryModel,
                             Image $imageModel, Game $gameModel, Tag $tagModel)
    {
...
//            $file->move('img', $file->getClientOriginalName());
            ...
            $imageModel->title = $nameImg;
            $imageModel->alt = $nameImg;
            $imageModel->save();
            ...
            } elseif (isset($request->tagId)) {
                $tag = $tagModel->where('id', $request->tagId)->get()->first();
                $tag->image()->save($imageModel);
...
//TODO delete from game and category
    public function deleteImage(Image $image, Request $request, Page $pageModel,
                                Category $categoryModel, Game $gameModel, Tag $tagModel)
    {
        if (isset($request->pageId)) {
            $page = $pageModel->where('id', $request->pageId)->get()->first();
            //dd($tag);
