...
class Parser
{
    ...
    public function getGameFromApi($gameUrl)
    {
        $gameUrl = 'https://gamedistribution.com/games/' . htmlspecialchars_decode($gameUrl, ENT_QUOTES);
...
//        $imgUrl = $imgOne->attr['src'];
        ...
        //game source
        $gameSourceContainer = $rawData->find('div.input-container', 0);
        if (is_null($gameSourceContainer)) {
//            dd($gameSourceContainer[0]);
            $gameSource = null;
        } else {
            $gameSource = $gameSourceContainer->find('input', 0)->attr['value'];
        }
//        dd($gameSourceContainer);


//        var_dump($gameSource);

        //        dd($gameSourceContainer->find('input')[0]->attr['value']);
        //tags
...
//        $desc = trim($desc);
    ...
    public function getGameFromUrl($gameURL)
    {
